...
;(function () {
  ...
  function initCustomizer() {
    ...
    const bwLabel = document.createElement('label'); bwLabel.append(bwCheckbox,' B&W');
    controls.appendChild(bwLabel);

    // Panels toggle
    const panelsCheckbox = document.createElement('input'); panelsCheckbox.type='checkbox';
    const panelsLabel = document.createElement('label'); panelsLabel.append(panelsCheckbox,' Show panels');
    ...
    function renderImage(variant, idx) {
      ...
      imgEl.style.width = '100%'; container.appendChild(imgEl);
      imgEl.onload = () => {
    ...
    panelsCheckbox.addEventListener('change', () => {
      if(panelsCheckbox.checked) drawPanels(); else clearPanels();
    });
    ...
    function drawPanels() {
      ...
      const totalInches = getWidthInches();
      const panelMax = unitSelect.value==='cm' ? 25*2.54 : 25;
      const count = Math.ceil(totalInches / panelMax);
      const wrap = container.querySelector('.cropper-container');
      wrap.style.position = 'relative';
      for(let i=1; i<count; i++){
        const x = data.left + (data.width/count)*i;
    ...
    // Apply transforms without resetting crop box
    function applyFlip() {
      const wrap = container.querySelector('.cropper-container');
      const sx = flipSelect.value==='horizontal' ? -1 : 1;
      const sy = flipSelect.value==='vertical' ? -1 : 1;
      if(wrap) wrap.style.transform = `scale(${sx},${sy})`;
    }
    function applyBW() {
      const wrap = container.querySelector('.cropper-container');
    ...
    function updateSizeAndPrice() {
      ...
      if(cropper && w>0 && h>0) {
        ...
        priceDiv.innerText = `Price: $${total.toFixed(2)}`;
    ...
    unitSelect.addEventListener('change', () => {
      ...
      [widthFeet,widthInches,heightFeet,heightInches].forEach(el => el.style.display = feetMode ? 'inline-block' : 'none');
      updateSizeAndPrice();
      if(panelsCheckbox.checked) drawPanels();
    });
    [widthInput,heightInput,widthFeet,widthInches,heightFeet,heightInches].forEach(el => el.addEventListener('input', () => {
      updateSizeAndPrice();
      if(panelsCheckbox.checked) drawPanels();
    }));
