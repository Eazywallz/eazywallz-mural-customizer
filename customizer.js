...
;(function () {
  ...
  function initCustomizer() {
    ...
    product.variants.forEach((v,i)=>{
      const o=document.createElement('option'); o.value=i; o.text=v.title; variantSelect.append(o);
    });
    ...
    function renderImage() {
      ...
      imgEl.onload = () => {
        ...
        cropper = new Cropper(imgEl, {
          ...
          ready() { updateAll(); if (panelsCheckbox.checked) drawPanels(); }
        });
      };
    }
    ...
    function applyFlip() {
      if (!cropper) return;
      // scaleX/scaleY Cropper methods
      cropper.scaleX(flipSelect.value==='horizontal' ? -1 : 1);
      cropper.scaleY(flipSelect.value==='vertical'   ? -1 : 1);
    }
    ...
    function drawPanels() {
      if (!cropper) return;
      const container = canvasArea.querySelector('.cropper-container');
      container.querySelectorAll('.panel-line').forEach(l=>l.remove());
      ...
      for (let i=1; i<count; i++) {
        ...
        Object.assign(line.style,{
          position:'absolute', top:`${cb.top}px`, left:`${x}px`,
          height:`${cb.height}px`, width:'2px',
          background:'rgba(255,0,0,0.7)', pointerEvents:'none'
        });
    ...
    openBtn.addEventListener('click', () => {
      ...
      // allow layout, then force cropper to resize
      setTimeout(() => {
