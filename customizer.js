...
;(function () {
  ...
  function initCustomizer() {
    ...
    Object.assign(modal.style, {
      ...
      overflow: 'auto', position: 'relative', padding: '1rem'
    });
    overlay.appendChild(modal);

    // Move container into modal
    ...
    [['inches','Inches'],['feet','Feet'],['cm','Centimeters']].forEach(([v,t]) => {
      const opt = document.createElement('option'); opt.value = v; opt.text = t; unitSelect.appendChild(opt);
    });
    ...
    [widthFeet, widthInches, heightFeet, heightInches].forEach(el => {
      el.style.display = 'none'; controls.appendChild(el);
    });

    // Flip selector
    const flipSelect = document.createElement('select');
    [['none','None'],['horizontal','Flip H'],['vertical','Flip V']].forEach(([v,t]) => {
      const opt = document.createElement('option'); opt.value = v; opt.text = t; flipSelect.appendChild(opt);
    });
    ...
    Object.assign(addBtn.style, {
      ...
      background: '#007bff', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer'
    });
    ...
    function updateAll() {
      ...
      if (cropper && w > 0 && h > 0) {
        ...
        const variant = product.variants[variantSelect.value];
        // variant.price is a string like "19.99"
    ...
    function drawPanels() {
      ...
      const maxPanel = unitSelect.value === 'cm' ? 25 * 2.54 : 25;
      const count = Math.ceil(total / maxPanel);
      const panelW = data.width / count;
      for (let i = 1; i < count; i++) {
        const x = data.left + panelW * i;
        const line = document.createElement('div');
        Object.assign(line.style, {
          position: 'absolute', top: `${data.top}px`, left: `${x}px`,
          height: `${data.height}px`, width: '2px', background: 'rgba(0,0,0,0.7)',
          pointerEvents: 'none'
        });
    ...
    unitSelect.addEventListener('change', () => {
      ...
      [widthInput, heightInput].forEach(e => e.style.display = isFeet ? 'none' : 'inline-block');
      [widthFeet, widthInches, heightFeet, heightInches].forEach(e => e.style.display = isFeet ? 'inline-block' : 'none');
      updateAll(); if (panelsCheckbox.checked) drawPanels();
    });
    [widthInput, heightInput, widthFeet, widthInches, heightFeet, heightInches].forEach(e =>
      e.addEventListener('input', () => { updateAll(); if (panelsCheckbox.checked) drawPanels(); })
    );
    ...
    panelsCheckbox.addEventListener('change', () => {
      panelsCheckbox.checked ? drawPanels() : container.querySelectorAll('.panel-line').forEach(el => el.remove());
    });
      ...
      cropper.getCroppedCanvas().toBlob(blob => {
        ...
        reader.onloadend = () => {
          ...
          fetch('/cart/add.js', {
            ...
            body: JSON.stringify({ id: variant.id, quantity: quantity, properties: props })
          })
            .then(r => {
