...
`;
    ...
    if (!modal) {
      ...
      [['none','None'],['horizontal','Flip H'],['vertical','Flip V']].forEach(([v,t])=>{const o=document.createElement('option');o.value=v;o.text=t;flipSelect.append(o);});
      controls.append(flipSelect);

      const bwCheckbox   = document.createElement('input'); bwCheckbox.type='checkbox'; controls.append(bwCheckbox, document.createTextNode(' B&W '));
      const panelsCheckbox = document.createElement('input'); panelsCheckbox.type='checkbox'; controls.append(panelsCheckbox, document.createTextNode(' Show panels '));
      ...
      function renderImage(){
        ...
        imgEl.onload = () => {
          ...
          cropper = new Cropper(imgEl,{viewMode:1,autoCropArea:1,dragMode:'move',cropBoxMovable:true,cropBoxResizable:false,zoomable:false,scalable:false,responsive:true});
          ['crop','cropmove','cropend'].forEach(evt=> cropper.on(evt,() => { if(panelsCheckbox.checked) drawPanels(); }));
      ...
      function drawPanels(){
        ...
        const total = getWidthInches(), maxW = 25, count = Math.ceil(total/maxW), step = data.width/count;
