  ...
  function initCustomizer() {
    ...
    function updateAll() {
      ...
      if (cropper && w > 0 && h > 0) {
        ...
        priceDiv.innerText = `Price: $${(unitPrice * area).toFixed(2)}`;
        if (qtyInput) qtyInput.value = area.toFixed(2);
      }
    }
    function applyFlip() {
      const wrap = container.querySelector('.cropper-container');
      if (wrap) wrap.style.transform = flipSelect.value === 'horizontal'
        ? 'scaleX(-1)'
    ...
    function drawPanels() {
      ...
      const totalW = getWidthInches();
      const maxPanel = unitSelect.value === 'cm' ? 25 * 2.54 : 25;
      const count = Math.ceil(totalW / maxPanel);
    ...
    renderImage();
    updateAll();
    if (!trigger) overlay.style.display = 'flex';

    console.log('Customizer initialized inside modal');
  }

  document.addEventListener('DOMContentLoaded', () => {
    loadCropper().then(initCustomizer).catch(err => console.error('Customizer failed to load:', err));
  });
})();
