...
;(function () {
  ...
  function initCustomizer() {
    ...
    // Add or re-add "Customize Mural" button
    ...
    if (!modal) {
      ...
      const heightInput = Object.assign(document.createElement('input'),{type:'number',placeholder:'Height',min:1}); controls.append(widthInput,heightInput);
      ...
      [['none','None'],['horizontal','Flip H'],['vertical','Flip V']].forEach(([v,t])=>{const o=document.createElement('option');o.value=v;o.text=t;flipSelect.appendChild(o);}); controls.appendChild(flipSelect);
      ...
      // Bind events inside this block (canvasArea retained via var)
      let cropper,imgEl;
      function clearCanvas(){ if(cropper)cropper.destroy(); canvasArea.innerHTML=''; }
      function renderImage(){ clearCanvas(); const variant=product.variants[variantSelect.value];
        let src=variant.image?.src||variant.featured_image?.src||product.images[0]; if(src.startsWith('//'))src=location.protocol+src;
        imgEl=document.createElement('img'); Object.assign(imgEl.style,{maxWidth:'100%',maxHeight:'100%',display:'block'});
        imgEl.src=src; imgEl.onload=()=>{ canvasArea.appendChild(imgEl);
      ...
      function getWidthInches(){return unitSelect.value==='feet'?((+widthFeet.value||0)*12+(+widthInches.value||0)):toInches(+widthInput.value||0);}
      function getHeightInches(){return unitSelect.value==='feet'?((+heightFeet.value||0)*12+(+heightInches.value||0)):toInches(+heightInput.value||0);}
      function updateAll(){ const w=getWidthInches(),h=getHeightInches(); if(cropper&&w>0&&h>0){cropper.setAspectRatio(w/h); const sqft=Math.ceil(w*h/144)||1; const rawCents=parseFloat(product.variants[variantSelect.value].price); const unitPrice=rawCents/100; priceDiv.innerText=`Price: $${(unitPrice*sqft).toFixed(2)}`; const qtyInputEl=document.querySelector('input[name="quantity"]'); if(qtyInputEl)qtyInputEl.value=sqft;} }
      ...
      function drawPanels(){ const wrap=canvasArea.querySelector('.cropper-container'); if(!cropper||!wrap)return; wrap.style.position='relative'; wrap.querySelectorAll('.panel-line').forEach(l=>l.remove()); const data=cropper.getCropBoxData(); const total=getWidthInches(),maxW=25; const count=Math.ceil(total/maxW),step=data.width/count; for(let i=1;i<count;i++){ const x=data.left+step*i; const line=document.createElement('div'); line.classList.add('panel-line'); Object.assign(line.style,{position:'absolute',top:`${data.top}px`,left:`${x}px`,height:`${data.height}px`,width:'2px',background:'rgba(255,0,0,0.7)',pointerEvents:'none'}); wrap.appendChild(line);} }

      openBtn.addEventListener('click',()=>overlay.style.display='flex');
      variantSelect.addEventListener('change',()=>{renderImage();applyFlip();applyBW();});
      unitSelect.addEventListener('change',()=>{ const feet=unitSelect.value==='feet'; widthInput.hidden=heightInput.hidden=feet; widthFeet.hidden=widthInches.hidden=heightFeet.hidden=heightInches.hidden=!feet; updateAll(); if(panelsCheckbox.checked)drawPanels(); });
